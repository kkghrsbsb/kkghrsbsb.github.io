...
export default function WriterPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('writer_session')
    if (stored) {
      setSession(stored)
      setReady(true)
    } else if (isDev) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (cover) {
        await workerSubmit(session!, `${dirPath}/${cover.name}`, cover.b64, `assets: cover for "${title}"`, true)
      }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <a href="/" style={{ display: 'flex', alignItems: 'center', opacity: 0.5,
          color: 'var(--foreground)', marginRight: '0.25rem' }} title="返回">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M19 12H5"/><path d="M12 19l-7-7 7-7"/>
          </svg>
        </a>
        <span style={{ fontWeight: 600, fontSize: '0.875rem', flex: 1 }}>新文章</span>
        <input ref={imgInputRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handleInlineImage} />
        <button onClick={() => imgInputRef.current?.click()}
          style={{ ...btn, padding: isMobile ? '8px' : '4px 10px' }} title="插入图片">
          {isMobile ? (
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <button onClick={handleLogout}
          style={{ ...btn, opacity: 0.5, padding: isMobile ? '8px' : '4px 10px' }} title="退出">
          {isMobile ? (
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div style={{
        ...
        {isMobile ? (
          <>
            ...
            <div onClick={() => document.getElementById('writer-tag-input')?.focus()}
              style={{
              ...
              <input id="writer-tag-input" value={tagInput}
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              ...
              <button onClick={() => coverInputRef.current?.click()}
                style={{ ...btn, flex: 1, padding: '8px 10px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <div onClick={() => document.getElementById('writer-tag-input')?.focus()}
                style={{
                  ...
                  padding: '3px 6px', border: '1px solid var(--border)', borderRadius: '0.375rem',
                  background: 'color-mix(in oklab, var(--muted) 50%, transparent)', cursor: 'text',
                }}>
                {tags.map((t) => (
                  <span key={t} style={{
